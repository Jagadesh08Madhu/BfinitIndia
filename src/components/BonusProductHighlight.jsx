...
export default function BonusProductHighlight() {
  ...
  useEffect(() => {
    fetch("https://api.exchangerate-api.com/v4/latest/USD")
      .then((res) => res.json())
      .then((data) => {
        const inrRate = data?.rates?.INR || 1;
      ...
      <div className="mt-16 grid grid-cols-1 gap-8 text-gray-700 md:grid-cols-2 md:gap-16">
        ...
        <div>
          ...
          <p className="mt-2 text-sm font-medium leading-relaxed">
            Unlock up to 5 free bonus products based on your hosting duration — a
            total value of up to{" "}
            <span className="font-semibold text-primary">
              ₹{(356.5 * exchangeRate).toFixed(2)}/year
            </span>
          </p>
        ...
        <div>
          ...
          <p className="mt-2 text-sm font-medium leading-relaxed">
            Unlock premium tools like login security, anti-spam, malware defense,
            and payroll software — worth up to{" "}
            <span className="font-semibold text-primary">
              ₹{(356.5 * exchangeRate).toFixed(2)}/year
            </span>{" "}
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex flex-col font-medium text-gray-900">
                    <p>
                      {item.name} – Gift Value: ₹
                      {(parseFloat(item.price.replace("€", "")) * exchangeRate).toFixed(2)}
